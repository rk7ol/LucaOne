...
S3_BASE_URI_DEFAULT="s3://plm-ml/code/lucaone"
...
# Keep uploads tight: only what inference needs.
aws "${aws_args[@]}" s3 sync "${SCRIPT_DIR}/src" "${base_uri%/}/src" \
  --only-show-errors "${aws_dryrun[@]}" \
  --exclude "*__pycache__*" --exclude "*.pyc"

aws "${aws_args[@]}" s3 sync "${SCRIPT_DIR}/config" "${base_uri%/}/config" --only-show-errors "${aws_dryrun[@]}" || true
aws "${aws_args[@]}" s3 sync "${SCRIPT_DIR}/vocab" "${base_uri%/}/vocab" --only-show-errors "${aws_dryrun[@]}" || true
